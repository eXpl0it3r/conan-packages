...
class PhysfsConan(ConanFile):
    ...
    def source(self):
        self.run("hg clone https://hg.icculus.org/icculus/physfs/ physfs")
        # This small hack might be useful to guarantee proper /MT /MD linkage
        # in MSVC if the packaged project doesn't have variables to set it properly
        tools.replace_in_file("physfs/CMakeLists.txt", "project(PhysicsFS)",
                              """project(PhysicsFS)
...
conan_basic_setup()""")
    ...
    def build(self):
        cmake = CMake(self)
        cmake.configure(source_folder="physfs")
        cmake.build()

    def package(self):
        self.copy("physfs.h", dst="include", src="physfs/src")
